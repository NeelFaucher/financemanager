...
import 'package:financemanager2/transactions.dart' as Transaction;
...
class NewTransactionState extends State<NewTransaction> {
  ...
  String formatterWithDay(DateTime date) {
  ...
  Transaction.Transaction getNewTransaction(String name, String category, double value) {
  ...
  String getDocumentId(String name, double value, DateTime date, String category){
    ...
    String dateString = formatterWithDay(date);
                  ...
                  children: [
                    ...
                    FloatingActionButton(
                      ...
                      onPressed: () {
                        ...
                        FirebaseFirestore.instance.collection('transactions').doc(getDocumentId(_budgetName.text, double.parse(_budgetPrice.text), DateTime.now(), categories[activeCategory])).set( {
                          'name': _budgetName.text,
                          'value': double.parse(_budgetPrice.text),
                          'category': categories[activeCategory],
                          'date': Timestamp.fromDate(DateTime.now()),
                        }
                        );
                        Transaction.Transaction newTransaction = getNewTransaction(
                            _budgetName.text,
                            categories[activeCategory],
                            double.parse(_budgetPrice.text));
                        providedTransactions.transactions.add(newTransaction);
                        providedTransactions.notifyListeners();


                        // if (providedTransactions.transactions.contains(newTransaction) == false){
                        //   providedTransactions.transactions.add(newTransaction);
                        // }
